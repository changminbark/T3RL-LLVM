...
PHASE2_JSON = os.environ.get(
    "PHASE2_JSON", str(ROOT / "results/phase2_k16/phase2_baseline.json")
)
SLM_GLOB = os.environ.get(
    "SLM_GLOB", str(ROOT / "results/probe_slm_qwen1p5b/*.rewrites.jsonl")
)
LLM_GLOB = os.environ.get(
    "LLM_GLOB", str(ROOT / "results/probe_llm_deepseek_k16/*.rewrites.jsonl")
)
...
ax1.set_title(
    "Phase 2 - best-of-K baseline (deepseek-v4-pro, 64 fns)\n"
...
slm_vals = [
    solve_at_k(slm, 8),
    slm_o.get("verified_faster", 0),
    slm_o.get("invalid_syntax", 0),
    slm_o.get("counterexample", 0),
]
llm_vals = [
    solve_at_k(llm, 8),
    llm_o.get("verified_faster", 0),
    llm_o.get("invalid_syntax", 0),
    llm_o.get("counterexample", 0),
]
...
b1 = ax.bar(
    x - w / 2,
    [v * 100 for v in slm_vals],
    w,
    label="SLM  qwen2.5-coder:1.5b",
    color="#ff7f0e",
)
b2 = ax.bar(
    x + w / 2,
    [v * 100 for v in llm_vals],
    w,
    label="LLM  deepseek-v4-pro",
    color="#2ca02c",
)
...
ax.set_title(
    "Does the prior resonate with capability? (same 64 fns, K=8)\n"
...
ax.set_title(
    "Where the wins are (best-of-8, by size|loops)\n"
...
ax.set_ylim(0, 35)
ax.set_title(
    "Base model vs base + LLVM tool (deepseek-v4-pro, 64 fns)\n"
