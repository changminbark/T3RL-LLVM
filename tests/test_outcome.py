...
def test_invalid_syntax_when_no_code():
    r = classify(
        ...
        0,
        GenFormat.ir,
        FixedVerifier(VerdictStatus.verified),
        StubPerf(),
    )
...
def test_counterexample_passthrough():
    r = classify(
        REC,
        _completion("define i32 @f(i32 %x){ ret i32 %x }"),
        0,
        GenFormat.ir,
        FixedVerifier(VerdictStatus.counterexample, cex="x=5"),
        StubPerf(),
    )
...
def test_timeout_passthrough():
    r = classify(
        ...
        0,
        GenFormat.ir,
        FixedVerifier(VerdictStatus.timeout),
        StubPerf(),
    )
